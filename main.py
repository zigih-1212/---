    ...
    async def dashboard(request: Request):
        ...
        try:
            ...
            payouts = conn.execute("""
                ...
                FROM payouts py LEFT JOIN users u ON py.user_id = u.user_id
...
<body>
    ...
    <div class="section">
        ...
        <p>Ожидают выплату: <b>{pending_payouts['cnt']}</b> заявок на сумму <b>{float(pending_payouts['total']):.2f} ₽</b></p>
    </div>
    <div class="section">
        <h2>💸 Заявки на выплату</h2>
        <table>
            <tr><th>#</th><th>Блогер</th><th>Карта</th><th>Блогеру</th><th>Вывести</th><th>Дата</th><th>Действие</th></tr>
            {payouts_rows or "<tr><td colspan='7'>Нет активных заявок</td></tr>"}
        </table>
    </div>
    <div class="section">
    ...
    async def user_card(request: Request, user_id: int):
        ...
        try:
            ...
            posts = conn.execute("SELECT * FROM posts WHERE user_id=? ORDER BY id DESC LIMIT 30", (user_id,)).fetchall()
        finally:
            conn.close()
        # Простая карточка пользователя (можно дополнить)
        html = f"""<html><body><h2>Пользователь {user_id}</h2>
        <p>Роль: {user['role']}</p>
        <p>Подписка до: {user['subscription_until']}</p>
        <a href="/admin/dashboard">← Назад</a></body></html>"""
        return HTMLResponse(html)
