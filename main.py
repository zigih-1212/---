...
async def admitad_postback(request: Request):
    ...
    conn = get_db()
    try:
        # subid1 – это наш идентификатор пользователя (формат username_uidID)
        # надо извлечь user_id
        import re
        ...
        if status in ("pending", "new"):
            ...
            # Перенос из pending в available и начисление approved
