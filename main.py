    ...
    if not user["oferta_accepted"]:
        if role == "saas":
            text_oferta = (
                ...
                "<i>Последняя редакция: 12 июля 2026 года</i>\n\n"
                ...
                "<b>2. Предмет</b>\n"
                "Вы получаете доступ к автопостингу товаров с партнёрскими ссылками. Сервис удерживает комиссию 5% от подтверждённого вознаграждения.\n\n"
                "<b>3. Учёт, выплаты и Налоги</b>\n"
                ...
                "• Выплата производится по запросу, за вычетом 5%. Пользователь самостоятельно декларирует свои доходы и платит налоги согласно закону своей страны.\n\n"
                "<b>4. Маркировка рекламы (ФЗ №38)</b>\n"
                "• Сервис автоматически вшивает в посты токен (erid) и наименование рекламодателя из CPA-сети.\n"
                "• Пользователь самостоятельно несет полную ответственность за ежемесячную передачу статистики просмотров постов в ОРД через свой личный кабинет. Сервис отчеты за Пользователя не сдает.\n\n"
                "<b>5. Запрещено</b>\n"
                "Спам, накрутка, самовыкупы, мотивированный трафик, брендовая реклама, размещение ссылок вне добавленных каналов. Запрещен контент, нарушающий Brand Safety (казино, пиратство, насилие). Нарушение – бан и обнуление баланса.\n\n"
                "<b>6. Ответственность</b>\n"
                "• Выплаты ограничены суммами, реально полученными от CPA-сети по SubID Пользователя.\n"
                "• Администрация вправе заморозить выплаты на время проверки трафика (до 90 дней)."
            )
        else:  # blogger
            text_oferta = (
                ...
                "<b>ПОЛЬЗОВАТЕЛЬСКОЕ СОГЛАШЕНИЕ (ПУБЛИЧНАЯ ОФЕРТА)</b>\n"
                "<i>Последняя редакция: 12 июля 2026 года</i>\n\n"
                "Нажимая «Принимаю», вы заключаете договор об оказании рекламных услуг.\n\n"
                "<b>1. Термины</b>\n"
                "• Сервис – данный Telegram-бот.\n"
                "• CPA-сеть – партнёрская сеть Admitad.\n"
                ...
                "Вы получаете бесплатный доступ к автопостингу. Доход от подтвержденных CPA-сетью заказов распределяется: 70% – Блогеру, 30% – Сервису.\n\n"
                "<b>3. Реферальная программа</b>\n"
                "• Приглашая блогеров по реферальной ссылке, вы получаете вознаграждение в размере 10% от суммы чистого заработка привлеченного лица.\n"
                "• Реферальные начисления выплачиваются строго ИЗ ДОЛИ СЕРВИСА (его 30%) по мере подтверждения заказов рекламодателями.\n\n"
                "<b>4. Учёт, выплаты и Налоги</b>\n"
                ...
                "• Блогер/Пользователь обязан самостоятельно декларировать доходы (иметь статус Самозанятого или ИП). Сервис не является налоговым агентом.\n\n"
                "<b>5. Маркировка рекламы (ФЗ №38)</b>\n"
                "• Сервис автоматически вшивает в посты токен (erid) и данные рекламодателя из CPA-сети.\n"
                "• Блогер самостоятельно несет полную юридическую ответственность за ежемесячное предоставление статистики просмотров постов в ОРД через свой личный кабинет. Сервис отчеты за Блогера не сдает.\n\n"
                "<b>6. Запрещенный трафик и Контент</b>\n"
                "Запрещены: спам, клик-фрод, самовыкупы, мотивированный трафик, брендовая реклама. Запрещен контент, нарушающий Brand Safety (казино, пиратство, насилие). Нарушение – блокировка и обнуление баланса.\n\n"
                "<b>7. Ответственность и Форс-мажор</b>\n"
                "• Выплаты ограничены суммами, реально поступившими на счет Сервиса по конкретному SubID Блогера.\n"
                "• При блокировке общего аккаунта в CPA-сети из-за фрода, выплаты замораживаются до разрешения спора. При подтверждении фрода баланс нарушителя аннулируется."
            )
...
async def handle_saas_channel_addition(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        try:
            ...
            if user and user["role"] != "blogger" and user["tariff_id"]:
