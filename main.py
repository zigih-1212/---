    ...
    if not user["oferta_accepted"]:
        if role == "saas":
            text_oferta = (
                ...
                "<b>2. Предмет</b>\n"
                "Вы получаете доступ к SaaS-инструменту автопостинга товаров с партнёрскими ссылками. "
                "Стоимость услуг Сервиса (Лицензионное вознаграждение) составляет 5% от суммы "
                "подтвержденного CPA-сетью вознаграждения по вашему SubID.\n\n"
                ...
                "• Перевод средств по запросу Пользователя производится за вычетом 5% стоимости услуг Сервиса. "
                "Пользователь обязан иметь статус Самозанятого или ИП и самостоятельно уплачивать налоги со своей доли (95%). "
                "Сервис не является налоговым агентом Пользователя.\n\n"
            ...
            text_oferta = (
                ...
                "• Трансфер средств производится по запросу. Блогер обязан самостоятельно "
                ...
                "в течение 24 часов. В случае отказа Сервис вправе заблокировать аккаунт Блогера."
            )
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def show_blogger_instruction(callback: CallbackQuery):
    text = (
        ...
        "Когда подписчик переходит по ссылке и покупает товар, в разделе «💰 Финансы» обновляется баланс:\n"
        ...
        "Когда накопится 3000 ₽, нажмите «💸 Запросить выплату» и укажите реквизиты карты.\n\n"
        "<b>🛡️ Шаг 3. Чек и защита выплаты</b>\n"
        "• Мы переводим деньги на вашу карту.\n"
        "• Бот меняет статус заявки на «Ожидание чека».<b>Вы обязаны в течение 24 часов</b> "
        "сформировать чек в приложении «Мой Налог» (тип: Продажа физ.лицу, услуга: Рекламные услуги) "
        "и отправить скриншот через кнопку «📤 Отправить чек» в разделе Финансы.\n"
        ...
        "4. Настройте интервал постов и шаблоны.\n"
        "5. Следите за доходом в «📊 Веб‑статистика».\n\n"
