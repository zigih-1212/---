...
def init_db() -> None:
    ...
    try:
        cursor.execute("ALTER TABLE cpc_campaigns ADD COLUMN more_rules TEXT DEFAULT ''")
    except sqlite3.OperationalError:
        pass
...
async def show_saas_instruction(callback: CallbackQuery):
    text = (
        ...
        "<b>🤖 Что делает бот?</b>\n"
        "Автоматически публикует товары из магазинов Admitad в ваш Telegram-канал "
        "с партнёрскими ссылками. Вы получаете <b>70%</b> от комиссии за каждую покупку.\n\n"

        "<b>🚀 Что такое Force Post?</b>\n"
        "Принудительная публикация одного поста <b>без задержек</b>. Нажмите «🚀 Force Post» "
        "в кабинете — бот сразу подберёт товар и опубликует. Без ожидания расписания.\n\n"

        "<b>⏱ Как работает таймер?</b>\n"
        "В веб-настройках (🛒 CPA магазины / 👆 CPC кампании) нажмите «⏱ Таймер» "
        "рядом с магазином или кампанией. Выберите дату и время — бот опубликует "
        "<b>точно в указанное время</b>. Таймер имеет приоритет над интервалом.\n\n"

        "<b>⏰ Интервал постов</b>\n"
        "Если таймер не настроен, бот публикует посты через равные промежутки времени. "
        "Настройте интервал в веб-настройках (⚙️ Настройки → Основные).\n\n"

        "<b>🗑 Автоудаление</b>\n"
        "Посты удаляются автоматически (по умолчанию через 7 дней). Можно изменить "
        "или отключить в настройках. Переходы засчитываются 30 дней даже после удаления.\n\n"

        "<b>📊 CPA и CPC — в чём разница?</b>\n"
        "• <b>CPA</b> (Покупка) — доход за совершённую покупку по вашей ссылке.\n"
        "• <b>CPC</b> (Клики) — доход за каждый переход по рекламной ссылке.\n"
        "Оба типа доступны в разделе «🏪 Магазины» в кабинете.\n\n"

        "<b>💳 Как вывести деньги?</b>\n"
        "Минимум <b>3000 ₽</b>. Нужен статус Самозанятого или ИП. "
        "Откройте веб-статистику → «💸 Запросить выплату» → укажите реквизиты. "
        "После получения — загрузите чек из «Мой Налог» за 24 часа.\n\n"

        "<b>📊 Отчёт для ОРД (ЕРИР)</b>\n"
        "Раз в месяц скачайте Excel-отчёт на странице статистики и загрузите "
        "в личный кабинет ОРД. Бот автоматически считает просмотры.\n\n"

        "<b>🔗 Реферальная программа</b>\n"
        "Приглашайте других — получайте <b>10%</b> от их дохода.\n\n"

        "<i>По вопросам: 👉 <a href='https://t.me/Zigih90'>@Zigih90</a></i>"
    )
    await safe_edit(callback.message, text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
def setup_scheduler(bot: Bot) -> AsyncIOScheduler:
    ...
    scheduler.add_job(publish_from_catalog, trigger="interval", minutes=10, jitter=30, kwargs={"bot": bot}, id="publish_catalog", replace_existing=True, max_instances=1, coalesce=True, misfire_grace_time=300)
    scheduler.add_job(publish_cpc_campaigns, trigger="interval", minutes=15, jitter=30, kwargs={"bot": bot}, id="publish_cpc", replace_existing=True, max_instances=1, coalesce=True, misfire_grace_time=300)
    scheduler.add_job(refill_admitad_catalogs, trigger="interval", minutes=15, id="refill_admitad", replace_existing=True, max_instances=1, coalesce=True, misfire_grace_time=300)
    ...
    scheduler.add_job(check_rss_and_publish, trigger="interval", minutes=15, kwargs={"bot": bot}, id="check_rss", replace_existing=True, max_instances=1, coalesce=True, misfire_grace_time=300)
