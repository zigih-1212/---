...
"""
    ...
    async def saas_card(request: Request, user_id: int):
        ...
        ban_btn = f"""<form action="/admin/saas/{user_id}/ban" method="post" style="display:inline">
...
async def process_saas_core(
    ...
    for user in saas_users:
        ...
        for ch in channels:
            ...
            affiliate_url = erid_data.get("link") or f"https://www.wildberries.ru/catalog/{sku}/detail.aspx"
            ...
            if msg:
                logger.info(f"✅ [DEBUG] Пост успешно опубликован в {channel_id}")
                conn = get_db()
                conn.execute("INSERT INTO posts (user_id, donor_post_id, channel_id, sku, erid, status) VALUES (?, ?, ?, ?, ?, 'published')",
...
# =============================================================================
# === ПРОВЕРКА БИЛЛИНГА (РАБОТАЕТ С SAAS) =====================================
# =============================================================================
    ...
    try:
        ...
        for row in expired_users:
            ...
            try:
                ...
                logger.error(f"Не удалось отправить уведомление об истечении подписки: {e}")
        conn.commit()
    finally:
        conn.close()

# =============================================================================
# === ПЛАНИРОВЩИК (SETUP_SCHEDULER) ===========================================
# =============================================================================
