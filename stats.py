...
def get_saas_overview(user_id: int) -> Dict:
    ...
    try:
        ...
        store_stats = conn.execute("""
            ...
            ORDER BY cnt DESC
        """, (user_id,)).fetchall()
        # Если связь через gdeslon_catalog не настроена, можно по donor_post_id содержащему 'admitad_'
        # Но проще пока взять из таблицы posts, если поле source там есть.
        # Надёжнее использовать таблицу gdeslon_catalog.
    finally:
        conn.close()
    return {
